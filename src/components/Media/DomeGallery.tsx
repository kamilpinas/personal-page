...
function buildItems(pool: ImageItem[], seg: number, t: TFunction): ItemDef[] {
  ...
  if (pool.length > totalSlots) {
    console.warn(
      t("domeGallery.warning.imageCountExceedsTiles", {
        imageCount: pool.length,
        availableTiles: totalSlots,
      })
    )
  }
  ...
  return (
    <>
      ...
      >
        <main
          ref={mainRef}
          className="absolute inset-0 grid place-items-center overflow-hidden select-none bg-bg"
          style={{
            touchAction: "none",
              ...
              {items.map((it, i) => (
                ...
                >
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    aria-label={it.alt || t("domeGallery.ariaLabel.openImage")}
                    onClick={(e) => {
